...
namespace org.GraphDefined.Vanaheimr.Hermod.HTTP
{
                                 ...
                                 IComparable
    {
        ...
        #region TryParse(Text, out KeepAliveType)

        /// <summary>
        /// Parse the text representation of a HTTP Keep Alive header field.
        /// </summary>
        /// <param name="Text">The string representation of a HTTP Keep Alive header field.</param>
        public static Boolean TryParse(String                                   Text,
                                       [NotNullWhen(true)]  out KeepAliveType?  KeepAliveType)
        {
                                       ...
                                       Select(v => v.Trim()))
            {
                ...
                if (property.Length == 2)
                    if (property[0] == "timeout")
                        if (UInt32.TryParse(property[1], out var keepAliveValue))
                        {
                            timeout = TimeSpan.FromSeconds(keepAliveValue);
                            continue;
                        }

                if (property.Length == 2)
                    if (property[0] == "max")
                        if (UInt32.TryParse(property[1], out var keepAliveMax))
                        {
                            maxNumberOfRequests = keepAliveMax;
                            continue;
                        }
            ...
            KeepAliveType = new KeepAliveType(
                                timeout,
                                maxNumberOfRequests
                            );
