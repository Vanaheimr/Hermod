...
namespace org.GraphDefined.Vanaheimr.Hermod.DNS
{
    ...
    public class CDS : ADNSResourceRecord
    {
        ...
        public static CDS? TryParseFromJSON(DomainName Name, TimeSpan TimeToLive, String Data)
        {
            try
            {
                var parts = Data.Split(' ', 4);
                if (parts.Length < 4) return null;
                return new CDS(Name, DNSQueryClasses.IN, TimeToLive,
        ...
        protected override String ZoneFileRData()
            => $"{KeyTag} {Algorithm} {DigestType} {Convert.ToHexString(Digest).ToLowerInvariant()}";
