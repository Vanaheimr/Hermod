...
namespace org.GraphDefined.Vanaheimr.Hermod.DNS
{
    ...
    public class ZONEMD : ADNSResourceRecord
    {
        ...
        public static ZONEMD? TryParseFromJSON(DomainName Name, TimeSpan TimeToLive, String Data)
        {
            try
            {
                var parts = Data.Split(' ', 4);
                if (parts.Length < 4) return null;
                return new ZONEMD(Name, DNSQueryClasses.IN, TimeToLive,
        ...
        protected override String ZoneFileRData()
            => $"{Serial} {Scheme} {HashAlgorithm} {Convert.ToHexString(Digest).ToLowerInvariant()}";
