...
namespace org.GraphDefined.Vanaheimr.Hermod.DNS
{
    ...
    public class HINFO : ADNSResourceRecord
    {
        ...
        public static HINFO? TryParseFromJSON(DomainName Name, TimeSpan TimeToLive, String Data)
        {
            try
            {
                var parts = Data.Split(' ', 2);
                if (parts.Length < 2) return null;
                return new HINFO(Name, DNSQueryClasses.IN, TimeToLive, parts[0].Trim('"'), parts[1].Trim('"'));
            }
            catch { return null; }
        }
