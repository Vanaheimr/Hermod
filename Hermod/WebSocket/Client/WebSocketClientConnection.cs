...
namespace org.GraphDefined.Vanaheimr.Hermod.WebSocket
{
                                             ...
                                             IEquatable<WebSocketClientConnection>
    {
        ...
        /// <summary>
        /// Whether the remote endpoint closed or reset the underlying TCP connection.
        /// A TCP FIN/RST is invisible to DataAvailable-polling: It can only be
        /// detected via Socket.Poll(SelectRead) while no data is available,
        /// or via a read operation returning zero bytes.
        /// </summary>
        public Boolean IsRemoteTCPConnectionClosed
        {
            get
            {
                try
                {
                    return tcpSocket.Poll(0, SelectMode.SelectRead) &&
                                           ...
                                           CancellationToken  CancellationToken = default)
        {
            ...
            if (MaxBackpressure > 0 && !isClosing)
            {
                ...
                if (projected > MaxBackpressure)
                {
                    ...
                    {
                        WebSocketClient.Logger.LogWarning(
            ...
            try
            {
                ...
                try
                {
                    writeTimeout = tcpStream.WriteTimeout > 0
                                       ? TimeSpan.FromMilliseconds(tcpStream.WriteTimeout)
                                       : DefaultSendTimeout;
                }
                catch
                {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        await httpStream.FlushAsync(timeoutCTS.Token);
        ...
        /// Unlike Read(...), a cancellation (e.g. a periodic wake-up to re-check the
        /// connection state) surfaces as an OperationCanceledException instead of being
        ...
        public ValueTask<Int32> ReadAsync(Memory<Byte>       Buffer,
            ...
            => httpStream.ReadAsync(Buffer, CancellationToken);
        ...
        /// Return a JSON representation of this object.
        /// </summary>
        /// <param name="CustomWebSocketClientConnectionSerializer">A delegate to serialize custom HTTP WebSocket client connections.</param>
                                          ...
                                          WebSocketClientConnection WebSocketConnection2)
        {
            ...
            if (WebSocketConnection1 is null)
                throw new ArgumentNullException(nameof(WebSocketConnection2), "The given HTTP WebSocket client connection must not be null!");
        ...
        public Int32 CompareTo(Object? Object)
            ...
            => Object is WebSocketClientConnection webSocketClientConnection
                   ...
                   : throw new ArgumentException("The given object is not a HTTP WebSocket client connection!",
                                                 nameof(Object));
        ...
        public Int32 CompareTo(WebSocketClientConnection? WebSocketConnection)
        {
            ...
            if (WebSocketConnection is null)
                throw new ArgumentNullException(nameof(WebSocketConnection), "The given HTTP WebSocket client connection must not be null!");
