...
namespace org.GraphDefined.Vanaheimr.Hermod.Tests.HTTP2
{
    ...
    public class StreamingBodyTests
    {
        ...
        public async Task OurClient_ServerStreamingResponse()
        {
            ...
            var sr   = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/stream-response");
            Assert.Multiple(() =>
            {
        ...
        {
            var conn = await HTTP2Client.ConnectAsync("localhost", srv.Port, H2.AcceptAnyServerCert);
            var echo = await conn.SendRequestAsync(HTTPMethod.POST, URIScheme.https, $"localhost:{srv.Port}", "/echo-stream",
        ...
        {
            var conn = await HTTP2Client.ConnectAsync("localhost", srv.Port, H2.AcceptAnyServerCert);
            var tr   = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/trailers");
            Assert.Multiple(() =>
            {
        ...
        public async Task OurClient_LargeStreamedRequestBody()
        {
            ...
            var bc = await conn.SendRequestAsync(HTTPMethod.POST, URIScheme.https, $"localhost:{srv.Port}", "/count-body", Body: big);
            Assert.Multiple(() =>
            {
