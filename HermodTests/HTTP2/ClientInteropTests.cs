...
namespace org.GraphDefined.Vanaheimr.Hermod.Tests.HTTP2
{
    ...
    public class ClientInteropTests
    {
        ...
        public async Task OurServer_RequestsAndFlowControl()
        {
            ...
            var get = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/");
            Assert.Multiple(() =>
            {
            ...
            var echo = await conn.SendRequestAsync(HTTPMethod.POST, URIScheme.https, $"localhost:{srv.Port}", "/echo", Body: payload);
            Assert.Multiple(() =>
            {
            ...
            var large = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/large");
            Assert.Multiple(() =>
            {
        ...
        public async Task OurServer_Multiplexing()
        {
            ...
            var tSlow  = conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/slow");
            var tFast  = conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/");
            var tLarge = conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/large");
        ...
        public async Task Kestrel_RequestsAndFlowControl()
        {
            ...
            var big = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/big");
            Assert.Multiple(() =>
            {
        ...
        public async Task Kestrel_ConcurrencyAndNotFound()
        {
            ...
            var r = await Task.WhenAll(
                conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/"),
                conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/big"),
                conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/"));
            Assert.That(r.All(x => x.Status == 200), Is.True, "3 concurrent requests all 200");

            var missing = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/does-not-exist");
