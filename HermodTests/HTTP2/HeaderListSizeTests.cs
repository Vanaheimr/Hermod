...
namespace org.GraphDefined.Vanaheimr.Hermod.Tests.HTTP2
{
    ...
    public class HeaderListSizeTests
    {
        ...
        public async Task Client_RefusesToSendMoreThanThePeerAllows()
        {
            ...
            var first = await conn.StartRequestAsync("GET", "https", authority, "/");
            await first.Response;

            // Comfortably under the advertised 8 KiB: unaffected.
            var modest = await conn.SendRequestAsync("GET", "https", authority, "/",
                             [("x-context", new String('c', 4000))]);

            Assert.That(async () => await conn.SendRequestAsync("GET", "https", authority, "/",
            ...
            var next = await conn.StartRequestAsync("GET", "https", authority, "/");
        ...
        public async Task Client_RefusesTrailersLargerThanThePeerAllows()
        {
            ...
            var conn   = await HTTP2Client.ConnectAsync("localhost", srv.Port, H2.AcceptAnyServerCert);
            var stream = await conn.StartStreamingRequestAsync("POST", "https", $"localhost:{srv.Port}", "/");
            ...
            var ok = await conn.StartStreamingRequestAsync("POST", "https", $"localhost:{srv.Port}", "/");
        ...
        public async Task Server_RefusesToSendMoreThanTheClientAllows()
        {
            ...
            var response = await conn.SendRequestAsync("GET", "https", $"localhost:{srv.Port}", "/");
