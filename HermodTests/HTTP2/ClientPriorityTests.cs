...
namespace org.GraphDefined.Vanaheimr.Hermod.Tests.HTTP2
{
    ...
    public class ClientPriorityTests
    {
        ...
        public async Task OurServer_PriorityEmissionAndUpdate()
        {
            ...
            var p0 = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/echo", Priority: new HTTP2Priority(0, true));
            var p5 = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/echo", Priority: new HTTP2Priority(5, false));
            var pNone = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/echo");
            Assert.Multiple(() =>
            {
            ...
            var handle = await conn.StartRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/slow");
            ...
            var after = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/echo");
        ...
        public async Task Kestrel_AcceptsPrioritySignals()
        {
            ...
            var withPriority = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/", Priority: new HTTP2Priority(1, false));
            Assert.Multiple(() =>
            {
            ...
            var handle = await conn.StartRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/");
            await conn.UpdatePriorityAsync(handle.StreamId, new HTTP2Priority(0, false));
            var resp = await handle.Response;
            Assert.That(resp.Status, Is.EqualTo(200), "Kestrel accepts a client PRIORITY_UPDATE");

            var follow = await conn.SendRequestAsync(HTTPMethod.GET, URIScheme.https, $"localhost:{srv.Port}", "/");
